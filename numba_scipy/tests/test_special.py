...
from numba_scipy.special.signatures import (
    parse_capsule_name,
...
NUMBA_TYPES_TO_TEST_POINTS = {
    ...
    numba.types.long_: np.array([-100, -10, -1, 0, 1, 10, 100], dtype=np.int_),
}

SKIP_LIST = {
    # Should be fixed by https://github.com/scipy/scipy/pull/10455
    ("hyperu", (numba.types.float64,) * 3),
    # Sometimes returns nan, sometimes returns inf. Likely a SciPy bug.
    ("eval_jacobi", (numba.types.float64,) * 4),
    # Sometimes returns nan, sometimes returns inf. Likely a SciPy bug.
    ("eval_sh_jacobi", (numba.types.float64,) * 4),
}
...
def test_get_signatures_from_pyx_capi(cysc_mock):
    ...
    mangled_name = "__pyx_fuse_0pdtr"
    cysc_mock.__pyx_capi__ = {mangled_name: capsule}
    expected = {
...
@patch(
    "numba_scipy.special.signatures.ctypes.CFUNCTYPE",
...
def test_ensure_signatures_generated():
    ...
    assert len(name_and_types_to_pointer) == len(signature_to_pointer)
...
def test_function(name, specialization):
    ...
    args = itertools.product(
        *(
            NUMBA_TYPES_TO_TEST_POINTS[numba_type]
            for numba_type in specialization
        )
    )
